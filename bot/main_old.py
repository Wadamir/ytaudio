	...
	for attempt in range(1, YTDLP_MAX_RETRIES + 1):
		try:
			...
			if error_code in ("403", "429"):
				...
				# --- log REAL error code ---
				from bot.db.db import log_youtube_error, count_today_youtube_errors
...
async def _process_job(job: dict):
	...
	try:
		await status_msg.edit_text(tr_user(user.id, "reading_info"))
		...
		try:
			...
			await status_msg.edit_text(tr_user(user.id, "failed_reading_info"))
		...
		warning_line = tr_user(user.id, "warning_patient")
		if duration >= LONG_WARNING_SECONDS:
			warning_line = tr_user(user.id, "warning_long_video")

		elif estimated_size >= LARGE_WARNING_MB:
			warning_line = tr_user(user.id, "warning_large_file")
		...
		if use_fast_path:
			...
			await status_msg.edit_text(tr_user(user.id, "download_fast"))
		...
		try:
			...
			await status_msg.edit_text(tr_user(user.id, "failed_download"))
		...
		if real_size_mb <= MAX_TG_AUDIO_EFFECTIVE_MB:
			try:
				...
				try:
					await status_msg.edit_text(
						tr_user(user.id, "failed_sending_audio")
					)
				except Exception:
		...
		if duration >= MAX_VIDEO_SECONDS:
			await status_msg.edit_text(
				tr_user(user.id, "warning_max_seconds")
			)		
		else:
			await status_msg.edit_text(
				tr_user(user.id, "warning_too_large_file")
			)
		...
		try:
			...
			thumb_url = info.get("thumbnail")
			caption = tr_user(user.id, "download_ready_caption", link=link, filename=filename_download, bot_caption=BOT_CAPTION, bot_username=BOT_USERNAME)
			...
			await status_msg.edit_text(tr_user(user.id, "failed_create_link"))
		...
		await status_msg.edit_text(tr_user(user.id, "failed_worker"))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
	...
	if not allowed:
		...
		await update.message.reply_text(
			(
				tr(context, "daily_limit_reached", plan=plan, limit=limit, reset_in=reset_in)
			),
			parse_mode="HTML",
		)
		return	
	...
	if not re.search(r"(youtube\.com|youtu\.be)", url):
		await update.message.reply_text(tr(context, "invalid_youtube_link"))
		return

	# 🔑 Status message
	status_msg = await update.message.reply_text(tr(context, "queue"))
	job = {
		"user": user,
		"url": url,
...
def tr_user(user_id: int, key: str, **kwargs):
